...
export async function deleteSolanaKeypair(): Promise<boolean> {
    try {
        const [keypairDeleted, masterKeyDeleted] = await Promise.all([
            deleteStoredKeypair(),
            deleteMasterKey()
        ]);

        if (!keypairDeleted) { console.warn('Failed to delete keypair') }
        if (!masterKeyDeleted) { console.warn('Failed to delete master key') }
