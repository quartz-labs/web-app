...
export default function TelegramModal({ closeModal }: TelegramModalProps) {
  ...
  return (
    <>
      <div className={styles.contentWrapper}>
        <h2 className={styles.heading} style={{ marginBottom: "55px" }}>
          Telegram Notifications
        </h2>

        <p style={{ marginBottom: "25px" }}>
          Send your wallet address to our Telegram bot{" "}
          <a href="https://telegram.me/QuartzLTVBot" target="_blank">
            @QuartzLTVBot
          </a>{" "}
          to receive Telegram notifications when your Quartz account health drops{" "}
          <span className={"no-wrap"}>close to 0%.</span>
        </p>

        <div className={styles.inputSection} style={{ marginBottom: "35px" }}>
          ...
          >
            ...
            {!displayCopied && (
              <>
                ...
                <Image src="/copy.svg" alt="Copy" height={23} width={23} />
              </>
            )}
          </button>
        </div>
        ...
        <p style={{ marginBottom: "55px" }}>
          Repaying your loan before reaching 0% health will mean you avoid any{" "}
          <span className={"no-wrap"}>liquidation penalties.</span>
        </p>
      </div>
      ...
      <div className={styles.buttons}>
        <button
          ...
          onClick={() => {
            window.open(`https://telegram.me/QuartzLTVBot`, "_blank");
          }}
        >
          Open Telegram
        </button>

        <button className={`glass-button ghost ${styles.mainButton}`} onClick={() => closeModal()}>
          Done
        </button>
      </div>
    </>
