...
export default function TxStatusPopup() {
    ...
    return (
      <div className={`${styles.popup} ${styles.error}`}>
        <div className={styles.heading}>
          <p className={styles.headingError}>Transaction timed out</p>
        </div>
  ...
  if (status === TxStatus.SIGNING)
    return (
      <div className={styles.popup}>
        ...
        <div className={styles.message}>
          <Image height="20" width="20" alt="" src="/wallet-grey.svg" style={{ marginRight: "5px" }} />
          <p>Waiting for wallet to sign transaction</p>
        </div>
      </div>
    );

  if (status === TxStatus.SIGN_REJECTED)
    return (
      <div className={styles.popup}>
        <div className={styles.heading}>
  ...
  if (status === TxStatus.FINALIZED)
    return (
      <div className={styles.popup}>
        ...
        <div className={styles.message}>
          <Image height="25" width="25" alt="" src="/checkmark.png" />
          <a href={explorerUrl} target="_blank">
            View on Solscan
          </a>
        </div>
  ...
  return (
    <div className={styles.popup}>
      <div className={styles.heading}>
        {status === TxStatus.SENT && <p>Processing transaction...</p>}

        {status === TxStatus.CONFIRMED && <p>Confirming transaction...</p>}
      </div>
