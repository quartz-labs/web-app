...
export const baseUnitToUi = (amountBase: number | BN, decimals: number): string => {
  ...
  if (decimals < 0 || decimals > 20) {
    // 20 is a safe upper limit for most tokens
...
export async function createAtaIfNeeded(connection: Connection, ata: PublicKey, authority: PublicKey, mint: PublicKey) {
...
export function getQuartzHealthFromDrift(driftHealth: number): number {
  ...
  return Math.floor(
    Math.min(
      100,
      Math.max(0, (driftHealth - QUARTZ_HEALTH_BUFFER_PERCENTAGE) / (1 - QUARTZ_HEALTH_BUFFER_PERCENTAGE / 100)),
    ),
  );
}
