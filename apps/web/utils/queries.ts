...
export function useDriftDataQuery() {
  ...
  const queryFn = async () => {
    ...
    if (
      responseJson.balances === undefined ||
      responseJson.withdrawLimits === undefined ||
      responseJson.rates === undefined ||
      responseJson.health === undefined
    ) {
