import { BASE_UNITS_PER_USDC, type MarketIndex } from "@/config/constants";
...
export function baseUnitToDecimal(baseUnit: number, marketIndex: MarketIndex): number {
    if (marketIndex === DRIFT_MARKET_INDEX_SOL) return baseUnit / LAMPORTS_PER_SOL;
    else if (marketIndex === DRIFT_MARKET_INDEX_USDC) return baseUnit / BASE_UNITS_PER_USDC;
    else throw new Error("Not implemented"); // TODO: Implement for multiple markets
}
